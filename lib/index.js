...
async function activate(context) {
    const run = server_1.Server.prepare();
    if (!run) {
    ...
    function disposeOnDeactivation(disposable) {
        context.subscriptions.push(disposable);
    }
    function registerCommand(name, f) {
        disposeOnDeactivation(coc_nvim_1.commands.registerCommand(name, f));
    }
    ...
    server_1.Server.start(allNotifications);
    ...
    registerCommand('rust-analyzer.collectGarbage', () => server_1.Server.client.sendRequest('rust-analyzer/collectGarbage', null));
    ...
    registerCommand('rust-analyzer.showReferences', (uri, position, locations) => {
        // TODO
        return coc_nvim_1.commands.executeCommand('editor.action.showReferences', coc_nvim_1.Uri.parse(uri), position, locations);
    });
    // Executing `cargo watch` provides us with inline diagnostics on save
    let provider;
    runnables_1.interactivelyStartCargoWatch(context).then(p => {
        provider = p;
    });
    registerCommand('rust-analyzer.startCargoWatch', () => {
        if (provider) {
            ...
            runnables_1.startCargoWatch(context).then(p => {
                provider = p;
            });
        }
    });
    ...
    registerCommand('rust-analyzer.reload', async () => {
        if (server_1.Server.client != null) {
            coc_nvim_1.workspace.showMessage(`Reloading rust-analyzer...`);
            await server_1.Server.client.stop();
            server_1.Server.start(allNotifications);
        }
    });
...
function deactivate() {
    if (!server_1.Server.client) {
        return Promise.resolve();
    }
    return server_1.Server.client.stop();
}
...
Object.defineProperty(exports, "__esModule", { value: true });
const analyzerStatus = __importStar(__webpack_require__(3));
exports.analyzerStatus = analyzerStatus;
const applySourceChange = __importStar(__webpack_require__(13));
exports.applySourceChange = applySourceChange;
// import * as inlayHints from './inlay_hints';
const joinLines = __importStar(__webpack_require__(40));
exports.joinLines = joinLines;
const matchingBrace = __importStar(__webpack_require__(41));
exports.matchingBrace = matchingBrace;
const onEnter = __importStar(__webpack_require__(42));
exports.onEnter = onEnter;
const parentModule = __importStar(__webpack_require__(43));
exports.parentModule = parentModule;
const runnables = __importStar(__webpack_require__(44));
exports.runnables = runnables;
const syntaxTree = __importStar(__webpack_require__(55));
exports.syntaxTree = syntaxTree;
const expandMacro = __importStar(__webpack_require__(56));
...
async function handler() {
    const ret = await server_1.Server.client.sendRequest('rust-analyzer/analyzerStatus', null);
...
const which_1 = __importDefault(__webpack_require__(6));
...
class Server {
    static prepare() {
        ...
        let folder = '.';
        if (coc_nvim_1.workspace.workspaceFolder.uri.length > 0) {
    ...
    static start(notificationHandlers) {
        ...
        Server.client = new coc_nvim_1.LanguageClient('rust-analyzer', 'Rust Analyzer Language Server', serverOptions, clientOptions);
...
Server.config = new config_1.Config();
...
class Config {
    ...
    userConfigChanged() {
        ...
        if (config.has('enableCargoWatchOnStartup')) {
            this.cargoWatchOptions.enableOnStartup = config.get('enableCargoWatchOnStartup', 'ask');
        }
        if (config.has('trace.cargo-watch')) {
            this.cargoWatchOptions.trace = config.get('trace.cargo-watch', 'off');
        }
        ...
        if (config.has('cargo-watch.ignore')) {
            this.cargoWatchOptions.ignore = config.get('cargo-watch.ignore', []);
        }
...
Object.defineProperty(exports, "__esModule", { value: true });
const vscode_jsonrpc_1 = __webpack_require__(15);
...
const coc_nvim_1 = __webpack_require__(1);
const server_1 = __webpack_require__(4);
const apply_source_change_1 = __webpack_require__(13);
async function handle() {
    const doc = await coc_nvim_1.workspace.document;
    if (doc.textDocument.languageId !== 'rust') {
        return;
    }
    const range = await coc_nvim_1.workspace.getSelectedRange('v', doc);
    if (!range) {
        return;
    }
    const param = {
    ...
    const change = await server_1.Server.client.sendRequest('rust-analyzer/joinLines', param);
    await apply_source_change_1.handle(change);
}
exports.handle = handle;
...
async function handle() {
    ...
    const response = await server_1.Server.client.sendRequest('rust-analyzer/findMatchingBrace', request);
...
async function handle() {
    ...
    coc_nvim_1.events.on('TextChangedI', async (bufnr) => {
        ...
        if (lastChar && lastChar === '/' && Date.now() - lastTS < 40) {
            ...
            const change = await server_1.Server.client.sendRequest('rust-analyzer/onEnter', request);
            if (!change) {
                return;
            }
            await apply_source_change_1.handle(change);
...
async function handle() {
    ...
    const response = await server_1.Server.client.sendRequest('rust-analyzer/parentModule', param);
    if (response.length > 0) {
...
const cargo_watch_1 = __webpack_require__(47);
...
async function handle() {
    ...
    const runnables = await server_1.Server.client.sendRequest('rust-analyzer/runnables', params);
    const items = [];
...
exports.handle = handle;
async function handleSingle(runnable) {
...
async function startCargoWatch(context) {
    ...
    if (stderr.includes('no such subcommand: `watch`')) {
        ...
        const install = await coc_nvim_1.workspace.showPrompt(msg);
        if (!install) {
            return;
        }
        await coc_nvim_1.workspace.runTerminalCommand(`cargo install cargo-watch`, '.', true);
        const output = await execPromise('cargo watch --version').catch(e => e);
        if (output.stderr !== '') {
            coc_nvim_1.workspace.showMessage(`Couldn't install \`cargo-\`watch: ${output.stderr}`, 'error');
            return;
        }
    }
    else if (code !== 0) {
        coc_nvim_1.workspace.showMessage(`\`cargo-watch\` failed with ${code}: ${stderr}`);
        return;
    }
    const provider = cargo_watch_1.registerCargoWatchProvider(context.subscriptions);
    if (provider) {
        provider.start();
    }
    return provider;
}
exports.startCargoWatch = startCargoWatch;
/**
 ...
 */
async function interactivelyStartCargoWatch(context) {
    if (server_1.Server.config.cargoWatchOptions.enableOnStartup === 'disabled') {
        return;
    }
    if (server_1.Server.config.cargoWatchOptions.enableOnStartup === 'ask') {
        const watch = await coc_nvim_1.workspace.showPrompt('Start watching changes with cargo? (Executes `cargo watch`, provides inline diagnostics');
        if (!watch) {
            return;
        }
    }
...
exports.interactivelyStartCargoWatch = interactivelyStartCargoWatch;
...
const server_1 = __webpack_require__(4);
const rust_1 = __webpack_require__(48);
const SuggestedFixCollection_1 = __importDefault(__webpack_require__(50));
const vscode_1 = __webpack_require__(51);
const processes_1 = __webpack_require__(52);
const line_buffer_1 = __webpack_require__(53);
const watch_status_1 = __webpack_require__(54);
class CargoWatchProvider {
    constructor() {
        this.diagnosticCollection = coc_nvim_1.languages.createDiagnosticCollection('rustc');
        this.statusDisplay = new watch_status_1.StatusDisplay(server_1.Server.config.cargoWatchOptions.command);
        this.outputChannel = coc_nvim_1.workspace.createOutputChannel('Cargo Watch Trace');
        // Track `rustc`'s suggested fixes so we can convert them to code actions
        this.suggestedFixCollection = new SuggestedFixCollection_1.default();
        this.codeActionDispose = coc_nvim_1.languages.registerCodeActionProvider([{ scheme: 'file', language: 'rust' }], this.suggestedFixCollection, server_1.Server.client.id, SuggestedFixCollection_1.default.PROVIDED_CODE_ACTION_KINDS);
    }
    start() {
        if (this.cargoProcess) {
            coc_nvim_1.workspace.showMessage(`Cargo Watch is already running`);
            return;
        }
        let args = server_1.Server.config.cargoWatchOptions.command + ' --all-targets --message-format json';
        if (server_1.Server.config.cargoWatchOptions.command.length > 0) {
            // Excape the double quote string:
            args += ' ' + server_1.Server.config.cargoWatchOptions.arguments;
        }
        // Windows handles arguments differently than the unix-likes, so we need to wrap the args in double quotes
        if (process.platform === 'win32') {
            args = '"' + args + '"';
        }
        const ignoreFlags = server_1.Server.config.cargoWatchOptions.ignore.reduce((flags, pattern) => [...flags, '--ignore', pattern], []);
        // Start the cargo watch with json message
        this.cargoProcess = child_process_1.spawn('cargo', ['watch', '-x', args, ...ignoreFlags], {
            stdio: ['ignore', 'pipe', 'pipe'],
            cwd: coc_nvim_1.workspace.rootPath,
            windowsVerbatimArguments: true
        });
        const stdoutData = new line_buffer_1.LineBuffer();
        this.cargoProcess.stdout.on('data', (s) => {
        ...
        const stderrData = new line_buffer_1.LineBuffer();
        this.cargoProcess.stderr.on('data', (s) => {
            stderrData.processOutput(s, line => {
                this.logError('Error on cargo-watch : {\n' + line + '}\n');
            });
        });
        this.cargoProcess.on('error', (err) => {
    ...
    stop() {
        if (this.cargoProcess) {
            ...
            processes_1.terminate(this.cargoProcess);
            this.cargoProcess = undefined;
        }
        else {
            coc_nvim_1.workspace.showMessage(`Cargo Watch is not running`);
        }
    }
    dispose() {
        this.stop();
    ...
    logInfo(line) {
        if (server_1.Server.config.cargoWatchOptions.trace === 'verbose') {
            this.outputChannel.append(line);
        }
    }
    logError(line) {
        if (server_1.Server.config.cargoWatchOptions.trace === 'error' || server_1.Server.config.cargoWatchOptions.trace === 'verbose') {
            this.outputChannel.append(line);
        }
    }
    parseLine(line) {
        ...
        if (data.reason === 'compiler-artifact') {
            ...
            const mapResult = rust_1.mapRustDiagnosticToVsCode(msg);
            if (!mapResult) {
                return;
            }
            ...
            const isDuplicate = diagnostics.some(d => vscode_1.areDiagnosticsEqual(d, diagnostic));
            if (isDuplicate) {
                return;
            }
...
exports.CargoWatchProvider = CargoWatchProvider;
function registerCargoWatchProvider(subscriptions) {
    let cargoExists = false;
    const cargoTomlFile = path.join(coc_nvim_1.workspace.rootPath, 'Cargo.toml');
    ...
    if (!cargoExists) {
        coc_nvim_1.workspace.showMessage(`Couldn\'t find \'Cargo.toml' in ${cargoTomlFile}`);
        return;
    }
...
exports.registerCargoWatchProvider = registerCargoWatchProvider;
...
const path = __importStar(__webpack_require__(7));
const vscode_languageserver_protocol_1 = __webpack_require__(14);
const SuggestedFix_1 = __importDefault(__webpack_require__(49));
var SuggestionApplicability;
(function (SuggestionApplicability) {
...
})(SuggestionApplicability = exports.SuggestionApplicability || (exports.SuggestionApplicability = {}));
/**
 * Converts a Rust level string to a VsCode severity
 */
function mapLevelToSeverity(s) {
    if (s === 'error') {
 ...
 */
function mapSpanToLocation(span) {
    const fileName = path.join(coc_nvim_1.workspace.rootPath || '', span.file_name);
    const fileUri = coc_nvim_1.Uri.file(fileName);
    const range = vscode_languageserver_protocol_1.Range.create(span.line_start - 1, span.column_start - 1, span.line_end - 1, span.column_end - 1);
    // TODO: fsPath ok?
    return vscode_languageserver_protocol_1.Location.create(fileUri.toString(), range);
}
/**
 ...
 */
function mapSecondarySpanToRelated(span) {
    if (!span.label) {
        // Nothing to label this with
        return;
    }
    const location = mapSpanToLocation(span);
    return vscode_languageserver_protocol_1.DiagnosticRelatedInformation.create(location, span.label);
}
/**
 * Determines if diagnostic is related to unused code
 */
function isUnusedOrUnnecessary(rd) {
    if (!rd.code) {
        return false;
    }
 ...
 *    a `related`.
 */
function mapRustChildDiagnostic(rd) {
    ...
    if (span && typeof span.suggested_replacement === 'string') {
        ...
        return {
            suggestedFix: new SuggestedFix_1.default(title, location, span.suggested_replacement, span.suggestion_applicability)
        };
    }
    else {
        const related = vscode_languageserver_protocol_1.DiagnosticRelatedInformation.create(location, rd.message);
        return { related };
    }
}
/**
 ...
 */
function mapRustDiagnosticToVsCode(rd) {
    ...
    const vd = vscode_languageserver_protocol_1.Diagnostic.create(location.range, rd.message, severity);
    let source = 'rustc';
    ...
    if (isUnusedOrUnnecessary(rd)) {
        vd.tags.push(vscode_languageserver_protocol_1.DiagnosticTag.Unnecessary);
    }
    if (isDeprecated(rd)) {
        vd.tags.push(vscode_languageserver_protocol_1.DiagnosticTag.Deprecated);
    }
    return {
        location,
        diagnostic: vd,
...
exports.mapRustDiagnosticToVsCode = mapRustDiagnosticToVsCode;
 ...
 */
class SuggestedFix {
    constructor(title, location, replacement, applicability = rust_1.SuggestionApplicability.Unspecified) {
        this.title = title;
    ...
    toCodeAction() {
        ...
        const codeAction = {
            ...
            isPrefered: this.applicability === rust_1.SuggestionApplicability.MachineApplicable
        };
        codeAction.diagnostics = [...this.diagnostics];
        return codeAction;
    }
}
exports.default = SuggestedFix;
...
class SuggestedFixCollection {
     ...
     * diagnostic.
     */
    addSuggestedFixForDiagnostic(suggestedFix, diagnostic) {
        const fileUriString = suggestedFix.location.uri.toString();
        const fileSuggestions = this.suggestedFixes.get(fileUriString) || [];
        const existingSuggestion = fileSuggestions.find(s => s.isEqual(suggestedFix));
...
exports.default = SuggestedFixCollection;
SuggestedFixCollection.PROVIDED_CODE_ACTION_KINDS = [vscode_languageserver_protocol_1.CodeActionKind.QuickFix];
...
function areDiagnosticsEqual(left, right) {
...
exports.areDiagnosticsEqual = areDiagnosticsEqual;
...
Object.defineProperty(exports, "__esModule", { value: true });
const cp = __importStar(__webpack_require__(45));
const path_1 = __webpack_require__(7);
const isWindows = process.platform === 'win32';
...
async function handler() {
    ...
    const ret = await server_1.Server.client.sendRequest('rust-analyzer/syntaxTree', param);
...
async function handler() {
    ...
    const expanded = await server_1.Server.client.sendRequest('rust-analyzer/expandMacro', param);
    if (!expanded) {
        return;
    }
    await coc_nvim_1.workspace.nvim.command('tabnew').then(async () => {
